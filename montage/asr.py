...
vc.silero_vad.min_silence_duration=0.12
vc.silero_vad.min_speech_duration=0.10
vc.silero_vad.max_speech_duration=8.0
vc.sample_rate=16000
vad=sherpa_onnx.VoiceActivityDetector(vc, buffer_size_in_seconds=60)

w=wave.open('audio16k.wav'); a=np.frombuffer(w.readframes(w.getnframes()),dtype=np.int16).astype(np.float32)/32768
win=512; segs=[]
...
for st,samples in segs:
    ...
    out.append({'start':round(st,3),'end':round(st+dur,3),'text':r.text,
                'tokens':list(r.tokens),'ts':[round(st+t,3) for t in r.timestamps]})
    print(f'[{st:6.2f} - {st+dur:6.2f}] {r.text}')
json.dump(out, open('asr.json','w'), ensure_ascii=False, indent=1)
print('\nСЕГМЕНТОВ:',len(out))
print('ПОЛНЫЙ ТЕКСТ:', ' '.join(o['text'] for o in out))
