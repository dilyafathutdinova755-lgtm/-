...
for idx in ['1','3']:
    ...
    w=wave.open(f'audio{idx}.wav'); a=np.frombuffer(w.readframes(w.getnframes()),dtype=np.int16).astype(np.float32)/32768
    win=512; segs=[]
    ...
    json.dump(out, open(f'asr{idx}.json','w'), ensure_ascii=False, indent=1)
    print(f'=== видео {idx} ===')
