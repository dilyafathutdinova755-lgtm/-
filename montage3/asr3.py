...
w=wave.open('audio16k_3.wav'); a=np.frombuffer(w.readframes(w.getnframes()),dtype=np.int16).astype(np.float32)/32768
win=512; segs=[]
...
json.dump(out, open('asr3.json','w'), ensure_ascii=False, indent=1)
print('\nСЕГМЕНТОВ:',len(out))
